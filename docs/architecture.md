...
| --- | --- |
| 名称 | altobid |
| 目标 | 本地部署小参数多模态模型，解答小学题类型验证码（算式、选择题、读图等） |
| 版本 | v0.1（设计稿） |
| 日期 | 2026-07-22 |
...
变化时，才把经过 **ROI resize** 的单张图片送入本地多模态模型进行推理，输出答案。
...
本工具面向「小学题类型验证码」（算式计算、选择题、读图判断等），不承诺覆盖所有验证码形态。
...
3. **量化方案（关键决策）**：原计划用 AWQ 预量化权重，但实测在 **Windows + torch 2.6**
   下，AWQ 的推理内核 `awq_ext`（预编译 wheel 仅 0.0.8/0.0.9，均针对旧 torch ABI）
   加载即报 DLL 符号不匹配，且 autoawq 已官方弃用（最后适配 transformers 4.51.3）。
   降版会与「优先用新版」冲突。故改用 **bitsandbytes NF4 4bit**：加载官方 fp16 权重后
   **运行时量化**，不依赖预量化权重与坏掉的 AWQ 内核，Windows 有官方 wheel。

   显存预算（实测 Qwen2.5-VL-3B-Instruct）：

   | 权重格式 | 合计显存 | 8GB 卡(4060) | 12GB 卡(4080) |
   | --- | --- | --- | --- |
   | fp16 原始 | ~8~9 GB | ✗ 易 OOM | ✓ |
   | **NF4 (bnb 4bit)** | **~2.5 GB（实测峰值 2.52）** | ✓ | ✓ |

   NF4 精度略低于 AWQ、推理略慢于 fp16，但对「小学题验证码」几乎无影响，**设为默认**；
   12GB 卡若想要最高精度可在配置里关量化（`quantization: none`）用 fp16。
...
- **System**：`你是一个视觉问答助手。直接回答图片中的问题，只输出最终答案，不要解释过程、不要单位符号、不要标点。`
- **User**：`回答图中的问题。` + 图像。

约束输出格式可显著降低 PostProcessor 的解析难度和 token 消耗。Qwen2.5-VL 具备原生视觉理解能力，
无需先 OCR 再文本推理，直接端到端输出答案。
...
- **NF4 量化优先**：降低显存至 ~2.5GB，8GB 卡也能跑，为其他程序留出空间。
