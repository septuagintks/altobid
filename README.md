...
1. **启动**：运行后会加载模型（约 15-20 秒），完成后进入静默等待
2. **框选**：按 **Ctrl+F1** 弹出全屏半透明覆盖层，拖拽框出验证码区域（按 Esc 取消）
3. **监控**：框选完成后，覆盖层消失，仅保留绿色细框标记区域，开始监控
4. **调整**：按住 **Ctrl** 后框线响应鼠标，可拖动框体或拖拽四角调整大小
5. **推理**：检测到验证码变化并稳定后，自动推理并复制到剪贴板（可选弹窗显示）
6. **停止**：按 **Ctrl+C** 停止程序
